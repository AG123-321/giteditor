window.onload = async function () {
  ...
  if (!params.has("code")) {
    alert(
      "error: 400, NO_PARAMETERS_SPECIFIED. do not try to reload this page. instead, go back to the homepage and try the login with github again."
    );
    history.back();
  } else {
    ...
    const response = await fetch(baseUrl + getTokenUrl)
      ...
      .then((text) => {
        ...
        if (!textResponse.get("access_token")) {
          alert(
            "error: 502 unable to get you logged in please try again. error_code: 502_api_bad_response"
          );
